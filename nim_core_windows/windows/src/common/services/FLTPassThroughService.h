...
private:
    void httpProxy(const flutter::EncodableMap* arguments, std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result);

private:
    void receivedHttpMsgCb(const std::string& from_accid, const std::string& body, uint64_t timestamp);
};
