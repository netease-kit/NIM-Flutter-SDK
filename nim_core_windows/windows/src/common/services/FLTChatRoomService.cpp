...
FLTChatRoomService::FLTChatRoomService() {
    m_serviceName = "ChatroomService";
}

void FLTChatRoomService::onMethodCalled(const std::string& method, const flutter::EncodableMap* arguments,
                                        std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!checkChatRoomInit()) {
        result->Error("", "", NimResult::getErrorResult(-1, "chatRoom init failed"));
        return;
    }
...
bool FLTChatRoomService::checkChatRoomInit() {
    if (!m_init) {
        ...
        nim_chatroom::ChatRoom::RegEnterCb(std::bind(&FLTChatRoomService::enterCallback, this, std::placeholders::_1, std::placeholders::_2,
                                                     std::placeholders::_3, std::placeholders::_4, std::placeholders::_5));
        nim_chatroom::ChatRoom::RegExitCb_2(
            std::bind(&FLTChatRoomService::exitCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        nim_chatroom::ChatRoom::RegReceiveMsgCb(
            std::bind(&FLTChatRoomService::receiveMsgCallback, this, std::placeholders::_1, std::placeholders::_2));
        nim_chatroom::ChatRoom::RegSendMsgAckCb(
            std::bind(&FLTChatRoomService::sendMsgAckCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        nim_chatroom::ChatRoom::RegLinkConditionCb(
            std::bind(&FLTChatRoomService::linkConditionCallback, this, std::placeholders::_1, std::placeholders::_2));
        nim_chatroom::ChatRoom::RegNotificationCb(
            std::bind(&FLTChatRoomService::notificationCallback, this, std::placeholders::_1, std::placeholders::_2));
    }

    return true;
}

void FLTChatRoomService::enterChatroom(const flutter::EncodableMap* arguments,
                                       std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            flutter::EncodableMap extensionMap = std::get<flutter::EncodableMap>(iter->second);
            nim_cpp_wrapper_util::Json::Value extensionValue;
            Convert::getInstance()->convertMap2Json(&extensionMap, extensionValue);
            std::cout << "extension: " << nim::GetJsonStringWithNoStyled(extensionValue);
            enterInfo.SetExt(extensionValue);
        } else if (iter->first == flutter::EncodableValue("notifyExtension")) {
            flutter::EncodableMap notifyExtensionMap = std::get<flutter::EncodableMap>(iter->second);
            ...
            flutter::EncodableList tags = std::get<flutter::EncodableList>(iter->second);
            for (auto tag : tags) {
            ...
            // 不支持的字段
        } else if(iter->first == flutter::EncodableValue("desktopIndependentModeConfig")) {
    ...
    if(useIndependentMode) {
        ...
        for (independent_iter; independent_iter != independentConfigMap.end(); ++independent_iter) {
            if (independent_iter->second.IsNull()) {
                continue;
            }
            if (independent_iter->first == flutter::EncodableValue("appKey")) {
                independentEnterInfo.app_key_ = std::get<std::string>(independent_iter->second);
            } else if (independent_iter->first == flutter::EncodableValue("linkAddresses")) {
                flutter::EncodableList linkAddresses = std::get<flutter::EncodableList>(independent_iter->second);
                for(auto address : linkAddresses) {
                    independentEnterInfo.address_.emplace_back(std::get<std::string>(address));
                }
            } else if (independent_iter->first == flutter::EncodableValue("account")) {
                independentEnterInfo.accid_ = std::get<std::string>(independent_iter->second);
                if(!independentEnterInfo.accid_.empty()) {
                    useAnonymousMode = false;
                }
            } else if (independent_iter->first == flutter::EncodableValue("token")) {
                independentEnterInfo.token_ =  std::get<std::string>(independent_iter->second);
            }
        }
        ...
        if(useAnonymousMode) {
            ...
            if (nim_chatroom::ChatRoom::AnonymousEnter(roomId, anonymity_info, enterInfo)) {
                m_enterResult = result;
            } else {
                YXLOG(Info) << "AnonymousEnter failed" << YXLOGEnd;
                result->Error("","",NimResult::getErrorResult(-1, "enterChatroom failed"));
            }
        } else {
            YXLOG(Info) << "start IndependentEnter..." << YXLOGEnd;
            if(nim_chatroom::ChatRoom::IndependentEnter(roomId, independentEnterInfo)) {
                m_enterResult = result;
            } else {
        ...
        nim::PluginIn::ChatRoomRequestEnterAsync(roomId, [=](int error_code, const std::string& strResult) {
            if (error_code != nim::kNIMResSuccess) {
                YXLOG(Info) << "ChatRoomRequestEnterAsync failed" << YXLOGEnd;
                result->Error("", "", NimResult::getErrorResult(error_code, "enterChatroom failed"));
                return;
            }
...
void FLTChatRoomService::exitChatroom(const flutter::EncodableMap* arguments,
                                      std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    if (iter != arguments->end()) {
        if (iter->second.IsNull()) {
            result->Error("", "", NimResult::getErrorResult(-1, "exitChatroom params error"));
            return;
        }
...
void FLTChatRoomService::createMessage(const flutter::EncodableMap* arguments,
                                       std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    if (iter != arguments->end()) {
        if (iter->second.IsNull()) {
            result->Error("", "", NimResult::getErrorResult(-1, "createChatRoomMessage params error"));
            return;
        }
    ...
    if (iter != arguments->end()) {
        ...
        if (strMessageType != "custom" && strMessageType != "text") {
            result->Error("", "", NimResult::getErrorResult(-1, "createChatRoomMessage params error, Only supports custom and text"));
            return;
        }
    }
    ...
    if (strMessageType == "custom") {
        ...
        if (iter != arguments->end()) {
            if (iter->second.IsNull()) {
                result->Error("", "", NimResult::getErrorResult(-1, "createChatRoomMessage params error, attachment is empty"));
                return;
            }
        ...
        if (iter != arguments->end()) {
            if (iter->second.IsNull()) {
                result->Error("", "", NimResult::getErrorResult(-1, "createChatRoomMessage params error, text is empty"));
                return;
            }
            ...
            YXLOG(Info) << "createMessage message.msg_attach_" << message.msg_attach_ << YXLOGEnd;
        }
    }
...
void FLTChatRoomService::sendMessage(const flutter::EncodableMap* arguments, std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    if (!convertDartMessageToNimMessage(message, arguments)) {
        result->Error("", "", NimResult::getErrorResult(-1, "send chatroomMessage failed"));
        return;
    }

    std::string strMessage = nim_chatroom::ChatRoom::CreateRoomMessage(message.msg_type_, message.client_msg_id_, message.msg_attach_,
                                                                       message.msg_body_, message.msg_setting_, message.timetag_, message.sub_type_);
...
void FLTChatRoomService::fetchMessageHistory(const flutter::EncodableMap* arguments,
                                             std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            flutter::EncodableList messageTypeList = std::get<flutter::EncodableList>(iter->second);
            for (auto messageType : messageTypeList) {
                std::string strMessageType = std::get<std::string>(messageType);
                ;
                nim::NIMMessageType type;
                Convert::getInstance()->convertDartStringToNIMEnum(strMessageType, Convert::getInstance()->getMessageType(), type);
                parameters.msg_types_.emplace_back(static_cast<nim_chatroom::NIMChatRoomMsgType>(type));
            }
        }
    }

    nim_chatroom::ChatRoom::GetMessageHistoryOnlineAsync(
        roomId, parameters, [=](int64_t room_id, int error_code, const std::list<nim_chatroom::ChatRoomMessage>& msgs) {
            if (error_code == nim::kNIMResSuccess) {
                ...
                result->Error("", "", NimResult::getErrorResult(error_code, "fetchMessageHistory failed"));
            }
        });
}

void FLTChatRoomService::fetchChatroomInfo(const flutter::EncodableMap* arguments,
                                           std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    if (iter != arguments->end()) {
        if (iter->second.IsNull()) {
            result->Error("", "", NimResult::getErrorResult(-1, "fetchChatroomInfo params error"));
            return;
        }
    ...
    uint64_t roomId = strtoull(strRoomId.c_str(), nullptr, 0);
    nim_chatroom::ChatRoom::GetInfoAsync(roomId, [=](int64_t room_id, int error_code, const nim_chatroom::ChatRoomInfo& info) {
        if (error_code == nim::kNIMResSuccess) {
            ...
            nim_cpp_wrapper_util::Json::Value value = Convert::getInstance()->getJsonValueFromJsonString(info.ext_);
            ...
            Convert::getInstance()->convertNIMEnumToDartString(info.queuelevel, Convert::getInstance()->getChatroomQueueModificationLevel(),
                                                               strQueuelevel);
            ...
            result->Error("", "", NimResult::getErrorResult(error_code, "fetchChatroomInfo failed"));
        }
    });
}

void FLTChatRoomService::updateChatroomInfo(const flutter::EncodableMap* arguments,
                                            std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            flutter::EncodableMap requestMap = std::get<flutter::EncodableMap>(iter->second);
            ...
            for (request_iter; request_iter != requestMap.end(); ++request_iter) {
                ...
                } else if (request_iter->first == flutter::EncodableValue("broadcastUrl")) {
                    roomInfo.broadcast_url_ = std::get<std::string>(request_iter->second);
                } else if (request_iter->first == flutter::EncodableValue("extension")) {
                    flutter::EncodableMap extensionMap = std::get<flutter::EncodableMap>(request_iter->second);
                    nim_cpp_wrapper_util::Json::Value value;
                    Convert::getInstance()->convertMap2Json(&extensionMap, value);
                    roomInfo.ext_ = nim::GetJsonStringWithNoStyled(value);
                } else if (request_iter->first == flutter::EncodableValue("queueModificationLevel")) {
                    std::string strLevel = std::get<std::string>(request_iter->second);
                    Convert::getInstance()->convertDartStringToNIMEnum(strLevel, Convert::getInstance()->getChatroomQueueModificationLevel(),
                                                                       roomInfo.queuelevel);
                }
            }
    ...
    nim_chatroom::ChatRoom::UpdateRoomInfoAsync(roomId, roomInfo, needNotify, strNotifyExtension, [=](int64_t room_id, int error_code) {
        if (error_code == nim::kNIMResSuccess) {
            result->Success(NimResult::getSuccessResult());
        } else {
            result->Error("", "", NimResult::getErrorResult(error_code, "updateChatroomInfo failed"));
        }
    });
}

void FLTChatRoomService::fetchChatroomMembers(const flutter::EncodableMap* arguments,
                                              std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            params.type_ = static_cast<nim_chatroom::NIMChatRoomGetMemberType>(std::get<int>(iter->second));
        } else if (iter->first == flutter::EncodableValue("limit")) {
    ...
    if (!lastMemberAccount.empty()) {
        ...
        nim_chatroom::ChatRoom::GetMemberInfoByIDsAsync(
            roomId, ids, [&, result](int64_t room_id, int error_code, const std::list<nim_chatroom::ChatRoomMemberInfo>& infos) {
                if (error_code == nim::kNIMResSuccess) {
                    if (infos.size() > 0) {
                        for (auto info : infos) {
                            params.timestamp_offset_ = info.enter_timetag_;
                            std::cout << "params.timestamp_offset_" << params.timestamp_offset_ << std::endl;
                            break;
                        }

                        nim_chatroom::ChatRoom::GetMembersOnlineAsync(
                            room_id, params, [=](int64_t room_id, int error_code, const std::list<nim_chatroom::ChatRoomMemberInfo>& infos_) {
                                if (error_code == nim::kNIMResSuccess) {
                                    flutter::EncodableMap result_map;
                                    flutter::EncodableList memberInfoList;
                                    for (auto info : infos_) {
                                        flutter::EncodableMap memberInfoMap = convertChatRoomMemberInfoToMap(info);
                                    ...
                                    result->Error("", "", NimResult::getErrorResult(error_code, "fetchChatroomMembers failed"));
                                }
                            });
                    } else {
                        result->Error("", "", NimResult::getErrorResult(-1, "fetchChatroomMembers failed, lastMemberAccount not found"));
                    }
                } else {
                    result->Error("", "", NimResult::getErrorResult(-1, "fetchChatroomMembers failed, lastMemberAccount not found"));
                }
        ...
        nim_chatroom::ChatRoom::GetMembersOnlineAsync(
            roomId, params, [=](int64_t room_id, int error_code, const std::list<nim_chatroom::ChatRoomMemberInfo>& infos_) {
                if (error_code == nim::kNIMResSuccess) {
                    flutter::EncodableMap result_map;
                    flutter::EncodableList memberInfoList;
                    for (auto info : infos_) {
...
void FLTChatRoomService::fetchChatroomMembersByAccount(const flutter::EncodableMap* arguments,
                                                       std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            flutter::EncodableList accountList = std::get<flutter::EncodableList>(iter->second);
    ...
    nim_chatroom::ChatRoom::GetMemberInfoByIDsAsync(
        roomId, ids, [=](int64_t room_id, int error_code, const std::list<nim_chatroom::ChatRoomMemberInfo>& infos) {
            if (error_code == nim::kNIMResSuccess) {
                flutter::EncodableMap result_map;
                flutter::EncodableList memberInfoList;
                for (auto info : infos) {
                ...
                result->Error("", "", NimResult::getErrorResult(error_code, "fetchChatroomMembersByAccount failed"));
            }
        });
}

void FLTChatRoomService::updateChatroomMyMemberInfo(const flutter::EncodableMap* arguments,
                                                    std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            for (request_iter; request_iter != requestMap.end(); ++request_iter) {
                ...
                if (request_iter->first == flutter::EncodableValue("nickname")) {
                    ...
                    if (needSave) {
                        ...
                        YXLOG(Info) << "updateChatroomMyMemberInfo needSave." << needSaveJson << YXLOGEnd;
                    }
                }
            }
        } else if (iter->first == flutter::EncodableValue("needNotify")) {
            ...
            strNotifyExtension = nim::GetJsonStringWithNoStyled(value);
            YXLOG(Info) << "updateChatroomMyMemberInfo needNotify." << strNotifyExtension << YXLOGEnd;
        }
    }
    ...
    nim_chatroom::ChatRoom::UpdateMyRoomRoleAsync(
        ...
        [=](int64_t room_id, int error_code) {
            if (error_code == nim::kNIMResSuccess) {
                ...
                result->Error("", "", NimResult::getErrorResult(error_code, "updateMyChatroomMemberInfo failed"));
            }
        },
        needSaveJson);
}

void FLTChatRoomService::markChatroomMemberInBlackList(const flutter::EncodableMap* arguments,
                                                       std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    setMemberAttributeOnlineAsync(arguments, result, nim_chatroom::kNIMChatRoomMemberAttributeBlackList);
}

void FLTChatRoomService::markChatroomMemberBeManager(const flutter::EncodableMap* arguments,
                                                     std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    setMemberAttributeOnlineAsync(arguments, result, nim_chatroom::kNIMChatRoomMemberAttributeAdminister);
}

void FLTChatRoomService::markChatroomMemberMuted(const flutter::EncodableMap* arguments,
                                                 std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    setMemberAttributeOnlineAsync(arguments, result, nim_chatroom::kNIMChatRoomMemberAttributeMuteList);
}

void FLTChatRoomService::markChatroomMemberBeNormal(const flutter::EncodableMap* arguments,
                                                    std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    setMemberAttributeOnlineAsync(arguments, result, nim_chatroom::kNIMChatRoomMemberAttributeNomalMember);
}

void FLTChatRoomService::kickChatroomMember(const flutter::EncodableMap* arguments,
                                            std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            YXLOG(Info) << "kickChatroomMember notifyExtension." << notify_ext << YXLOGEnd;
        }
    }

    nim_chatroom::ChatRoom::KickMemberAsync(roomId, userId, notify_ext, [=](int64_t room_id, int error_code) {
        if (error_code == nim::kNIMResSuccess) {
            result->Success(NimResult::getSuccessResult());
        } else {
            result->Error("", "", NimResult::getErrorResult(error_code, "kickChatroomMember failed"));
        }
    });
}

void FLTChatRoomService::markChatroomMemberTempMuted(const flutter::EncodableMap* arguments,
                                                     std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("duration")) {
            ...
            flutter::EncodableMap optionsMap = std::get<flutter::EncodableMap>(iter->second);
            ...
            for (options_iter; options_iter != optionsMap.end(); ++options_iter) {
                ...
                if (options_iter->first == flutter::EncodableValue("roomId")) {
                    roomId = strtoull(std::get<std::string>(options_iter->second).c_str(), nullptr, 0);
                } else if (options_iter->first == flutter::EncodableValue("account")) {
                    userId = std::get<std::string>(options_iter->second);
                } else if (options_iter->first == flutter::EncodableValue("notifyExtension")) {
                    flutter::EncodableMap notifyExtensionMap = std::get<flutter::EncodableMap>(options_iter->second);
                    nim_cpp_wrapper_util::Json::Value value;
                    Convert::getInstance()->convertMap2Json(&notifyExtensionMap, value);
                    YXLOG(Info) << "markChatroomMemberTempMuted notifyExtension." << YXLOGEnd;
                    notify_ext = nim::GetJsonStringWithNoStyled(value);
                    YXLOG(Info) << "markChatroomMemberTempMuted notifyExtension." << notify_ext << YXLOGEnd;
                }
            }
        } else if (iter->first == flutter::EncodableValue("needNotify")) {
            needNotify = std::get<bool>(iter->second);
    ...
    nim_chatroom::ChatRoom::TempMuteMemberAsync(
        roomId, userId, duration, needNotify, notify_ext, [=](int64_t room_id, int error_code, const nim_chatroom::ChatRoomMemberInfo& info) {
            if (error_code == nim::kNIMResSuccess) {
                result->Success(NimResult::getSuccessResult());
            } else {
                result->Error("", "", NimResult::getErrorResult(error_code, "markChatroomMemberTempMuted failed"));
            }
        });
}

void FLTChatRoomService::fetchChatroomQueue(const flutter::EncodableMap* arguments,
                                            std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    if (iter != arguments->end()) {
        if (iter->second.IsNull()) {
            result->Error("", "", NimResult::getErrorResult(-1, "fetchChatroomQueue params error"));
            return;
        }
    ...
    nim_chatroom::ChatRoom::QueueListAsync(roomId, [=](int64_t room_id, int error_code, const nim_chatroom::ChatRoomQueue& queue) {
        if (error_code == nim::kNIMResSuccess) {
            ...
            for (auto chatRoomQueueElement : queue) {
                ...
                elementMap.insert(std::make_pair("value", chatRoomQueueElement.value_));
            ...
            result->Error("", "", NimResult::getErrorResult(error_code, "fetchChatroomQueue failed"));
        }
    });
}

void FLTChatRoomService::updateChatroomQueueEntry(const flutter::EncodableMap* arguments,
                                                  std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            flutter::EncodableMap entryMap = std::get<flutter::EncodableMap>(iter->second);
    ...
    nim_chatroom::ChatRoom::QueueOfferAsync(
        ...
        [=](int64_t room_id, int error_code) {
            if (error_code == nim::kNIMResSuccess) {
                ...
                result->Error("", "", NimResult::getErrorResult(error_code, "updateChatroomQueueEntry failed"));
            }
        },
        json_extension);
}

void FLTChatRoomService::batchUpdateChatroomQueue(const flutter::EncodableMap* arguments,
                                                  std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("roomId")) {
            ...
            flutter::EncodableList entryList = std::get<flutter::EncodableList>(iter->second);
            for (auto entry : entryList) {
                ...
                for (entry_iter; entry_iter != entryMap.end(); ++entry_iter) {
                    ...
                    members.members_values_.insert(std::make_pair(element.key_, element.value_));
                }
            }
            ...
            YXLOG(Info) << "batchUpdateChatroomQueue notifyExtension." << notifyExtension << YXLOGEnd;
        }
    }

    nim_chatroom::ChatRoom::QueueBatchUpdateAsync(
        roomId, members, needNotify, notifyExtension, [=](int64_t room_id, int error_code, const std::list<std::string>& not_in_queue) {
            if (error_code == nim::kNIMResSuccess) {
                ...
                result->Error("", "", NimResult::getErrorResult(error_code, "batchUpdateChatroomQueue failed"));
            }
        });
}

void FLTChatRoomService::pollChatroomQueueEntry(const flutter::EncodableMap* arguments,
                                                std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    nim_chatroom::ChatRoom::QueuePollAsync(roomId, key, [=](int64_t room_id, int error_code, const nim_chatroom::ChatRoomQueueElement& element) {
        if (error_code == nim::kNIMResSuccess) {
            ...
            result->Error("", "", NimResult::getErrorResult(error_code, "pollChatroomQueueEntry failed"));
        }
    });
}

void FLTChatRoomService::clearChatroomQueue(const flutter::EncodableMap* arguments,
                                            std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    if (iter != arguments->end()) {
        if (iter->second.IsNull()) {
            result->Error("", "", NimResult::getErrorResult(-1, "clearChatroomQueue params error"));
            return;
        }
    ...
    nim_chatroom::ChatRoom::QueueDropAsync(roomId, [=](int64_t room_id, int error_code) {
        if (error_code == nim::kNIMResSuccess) {
            ...
            result->Error("", "", NimResult::getErrorResult(error_code, "clearChatroomQueue failed"));
        }
    });
}

void FLTChatRoomService::enterCallback(int64_t room_id, const nim_chatroom::NIMChatRoomEnterStep step, int error_code,
    ...
    } else if (step == nim_chatroom::kNIMChatRoomEnterStepServerConnectOver && error_code != nim::kNIMResSuccess) {
    ...
    if (error_code != nim::kNIMResSuccess) {
        ...
        m_enterResult->Error("", "", NimResult::getErrorResult(error_code, "enterChatroom failed"));
        m_enterResult.reset();
        return;
    }
    ...
    std::string strQueuelevel = "";
    Convert::getInstance()->convertNIMEnumToDartString(info.queuelevel, Convert::getInstance()->getChatroomQueueModificationLevel(), strQueuelevel);
    chatRoomInfo.insert(std::make_pair("queueModificationLevel", strQueuelevel));
...
void FLTChatRoomService::exitCallback(int64_t room_id, int error_code, const nim_chatroom::NIMChatRoomExitReasonInfo& exit_info) {
    std::ostringstream stringstream;
    stringstream << room_id;
    std::string strRoomId = stringstream.str();
    flutter::EncodableMap arguments;
    ...
    flutter::EncodableMap extension;
    nim_cpp_wrapper_util::Json::Value value = Convert::getInstance()->getJsonValueFromJsonString(exit_info.notify_ext_);
    Convert::getInstance()->convertJson2Map(extension, value);
    ...
    } else if (exit_info.code_ == nim_chatroom::kNIMChatRoomExitReasonKickByMultiSpot) {
        arguments.insert(std::make_pair("reason", "byConflictLogin"));
        notifyEvent("onKickOut", arguments);
    } else if (exit_info.code_ == nim_chatroom::kNIMChatRoomExitReasonRoomInvalid) {
        arguments.insert(std::make_pair("reason", "dismissed"));
        notifyEvent("onKickOut", arguments);
    } else if (exit_info.code_ == nim_chatroom::kNIMChatRoomExitReasonBeBlacklisted) {
    ...
    if (error_code == nim::kNIMResSuccess) {
        ...
        m_exitResult->Error("", "", NimResult::getErrorResult(error_code, "exitChatroom failed"));
    }
...
void FLTChatRoomService::receiveMsgCallback(int64_t room_id, const nim_chatroom::ChatRoomMessage& result) {
    flutter::EncodableMap message;
    flutter::EncodableList messageList;
    flutter::EncodableMap arguments;

    YXLOG(Info) << "receiveMsgCallback result.msg_attach_: " << result.msg_attach_ << YXLOGEnd;
    YXLOG(Info) << "receiveMsgCallback result.msg_type_: " << result.msg_type_ << YXLOGEnd;
...
void FLTChatRoomService::sendMsgAckCallback(int64_t room_id, int error_code, const nim_chatroom::ChatRoomMessage& result) {
    YXLOG(Info) << "FLTChatRoomService::sendMsgAckCallback" << result.msg_attach_ << YXLOGEnd;
    std::cout << "FLTChatRoomService::sendMsgAckCallback:" << result.msg_attach_ << std::endl;
    ...
    if (iter != m_sendMsgResultMap.end()) {
        ...
        if (methonRsult) {
            if (error_code == nim::kNIMResSuccess) {
                ...
                methonRsult->Error("", "", NimResult::getErrorResult(error_code, "send chatroomMessage failed"));
            }
...
void FLTChatRoomService::linkConditionCallback(int64_t room_id, const nim_chatroom::NIMChatRoomLinkCondition& condition) {
...
void FLTChatRoomService::notificationCallback(int64_t room_id, const nim_chatroom::ChatRoomNotification& notification) {
    flutter::EncodableMap message;
    flutter::EncodableList messageList;
    flutter::EncodableMap arguments;
    ...
    if(notification.id_ == nim_chatroom::kNIMChatRoomNotificationIdQueueChanged) {
        ...
        attachMap.insert(std::make_pair("content", queueChangedNotification.value_));
        flutter::EncodableMap contentMap;
        contentMap.insert(std::make_pair(queueChangedNotification.key_, queueChangedNotification.value_));
    ...
    } else if(notification.id_ == nim_chatroom::kNIMChatRoomNotificationIdQueueBatchChanged) {
        nim_chatroom::ChatRoomQueueBatchChangedNotification queueBatchChangedNotification;
        ...
        attachMap.insert(std::make_pair("content", queueBatchChangedNotification.value_));
        flutter::EncodableMap contentMap;
    ...
    YXLOG(Info) << "notificationCallback notification.ext_:" << notification.ext_ << YXLOGEnd;
    flutter::EncodableMap extensionMap;
    nim_cpp_wrapper_util::Json::Value values = Convert::getInstance()->getJsonValueFromJsonString(notification.ext_);
    ...
    attachMap.insert(std::make_pair("tempMutedDuration", notification.temp_mute_duration_));
...
flutter::EncodableMap FLTChatRoomService::convertChatRoomMemberInfoToMap(const nim_chatroom::ChatRoomMemberInfo& my_info) {
    ...
    flutter::EncodableMap extension;
    nim_cpp_wrapper_util::Json::Value value = Convert::getInstance()->getJsonValueFromJsonString(my_info.ext_);
    Convert::getInstance()->convertJson2Map(extension, value);
    memberInfo.insert(std::make_pair("extension", extension));
    memberInfo.insert(std::make_pair("isOnline", my_info.state_ == nim_chatroom::kNIMChatRoomOnlineStateOnline));
    ...
    memberInfo.insert(std::make_pair("tempMuteDuration", my_info.temp_muted_duration_));
    ...
    Convert::getInstance()->convertNIMEnumToDartString(my_info.type_, Convert::getInstance()->getChatroomMemberType(), strMemberType);
    memberInfo.insert(std::make_pair("memberType", strMemberType));

    return memberInfo;
}

void FLTChatRoomService::setMemberAttributeOnlineAsync(const flutter::EncodableMap* arguments,
                                                       std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result,
        ...
        if (iter->first == flutter::EncodableValue("isAdd")) {
            ...
            for (options_iter; options_iter != optionsMap.end(); ++options_iter) {
                ...
                if (options_iter->first == flutter::EncodableValue("roomId")) {
                    ...
                    nim_cpp_wrapper_util::Json::Value value;
                    Convert::getInstance()->convertMap2Json(&notifyExtensionMap, value);
                    YXLOG(Info) << "setMemberAttributeOnlineAsync notifyExtension." << YXLOGEnd;
                    parameters.notify_ext_ = nim::GetJsonStringWithNoStyled(value);
                    YXLOG(Info) << "setMemberAttributeOnlineAsync notifyExtension." << parameters.notify_ext_ << YXLOGEnd;
                }
            }
        }
    }
    ...
    nim_chatroom::ChatRoom::SetMemberAttributeOnlineAsync(
        roomId, parameters, [=](int64_t room_id, int error_code, const nim_chatroom::ChatRoomMemberInfo& info) {
            if (error_code == nim::kNIMResSuccess) {
                flutter::EncodableMap memberInfoMap = convertChatRoomMemberInfoToMap(info);
                result->Success(NimResult::getSuccessResult(memberInfoMap));
            } else {
                result->Error("", "", NimResult::getErrorResult(error_code, "setMemberAttributeOnlineAsync failed"));
            }
        });
}

bool FLTChatRoomService::convertDartMessageToNimMessage(nim_chatroom::ChatRoomMessage& message, const flutter::EncodableMap* arguments) {
    if (!arguments) {
        return false;
    }
    ...
    flutter::EncodableMap messageMap = std::get<flutter::EncodableMap>(messageIt->second);
    ...
    for (iter; iter != messageMap.end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("messageId")) {
            ...
            if (message.msg_type_ == nim_chatroom::kNIMChatRoomMsgTypeCustom) {
                flutter::EncodableMap attachment = std::get<flutter::EncodableMap>(iter->second);
                nim_cpp_wrapper_util::Json::Value value;
                Convert::getInstance()->convertMap2Json(&attachment, value);
                message.msg_attach_ = nim::GetJsonStringWithNoStyled(value);
            }
            ...
            Convert::getInstance()->convertDartStringToNIMEnum(strSenderClientType, Convert::getInstance()->getClientType(), type);
            message.from_client_type_ = static_cast<nim_chatroom::NIMChatRoomClientType>(type);
        } else if (iter->first == flutter::EncodableValue("antiSpamOption")) {
            flutter::EncodableMap antiSpamOption = std::get<flutter::EncodableMap>(iter->second);
            auto antiSpamOptionIt = antiSpamOption.begin();
            for (antiSpamOptionIt; antiSpamOptionIt != antiSpamOption.end(); ++antiSpamOptionIt) {
                if (antiSpamOptionIt->second.IsNull()) {
                    continue;
                }
                if (antiSpamOptionIt->first == flutter::EncodableValue("enable")) {
                    message.msg_setting_.anti_spam_enable_ = std::get<bool>(antiSpamOptionIt->second);
                } else if (antiSpamOptionIt->first == flutter::EncodableValue("content")) {
                    message.msg_setting_.anti_spam_content_ = std::get<std::string>(antiSpamOptionIt->second);
                } else if (antiSpamOptionIt->first == flutter::EncodableValue("antiSpamConfigId")) {
                    message.msg_setting_.anti_spam_bizid_ = std::get<std::string>(antiSpamOptionIt->second);
                }
            }
        } else if (iter->first == flutter::EncodableValue("yidunAntiCheating")) {
            flutter::EncodableMap yidunAntiCheating = std::get<flutter::EncodableMap>(iter->second);
            nim_cpp_wrapper_util::Json::Value yidunAntiCheating_;
            if (!Convert::getInstance()->convertMap2Json(&yidunAntiCheating, yidunAntiCheating_)) {
                return false;
            }
...
void FLTChatRoomService::convertNimMessageToDartMessage(const nim_chatroom::ChatRoomMessage& message, flutter::EncodableMap& arguments) {
    arguments.insert(std::make_pair("messageId", message.client_msg_id_));

    std::string strMessageType = "";
    Convert::getInstance()->convertNIMEnumToDartString(static_cast<nim::NIMMessageType>(message.msg_type_), Convert::getInstance()->getMessageType(),
                                                       strMessageType);
    arguments.insert(std::make_pair("messageType", strMessageType));

    std::string strMessageSubType = "";
    Convert::getInstance()->convertNIMEnumToDartString(static_cast<nim::NIMMessageType>(message.sub_type_), Convert::getInstance()->getMessageType(),
                                                       strMessageSubType);
    ...
    arguments.insert(std::make_pair("isHighPriorityMessage", message.msg_setting_.high_priority_ == 1));
    arguments.insert(std::make_pair("enableHistory", message.msg_setting_.history_save_));
    ...
    config.insert(std::make_pair("enableHistory", message.msg_setting_.history_save_));
    ...
    Convert::getInstance()->convertNIMEnumToDartString(static_cast<nim::NIMClientType>(message.from_client_type_),
    ...
    antiSpamOption.insert(std::make_pair("enable", message.msg_setting_.anti_spam_enable_));
    antiSpamOption.insert(std::make_pair("content", message.msg_setting_.anti_spam_content_));
    antiSpamOption.insert(std::make_pair("antiSpamConfigId", message.msg_setting_.anti_spam_bizid_));
    ...
    if (!message.msg_setting_.yidun_anti_cheating_.empty()) {
        ...
        nim_cpp_wrapper_util::Json::Value values = Convert::getInstance()->getJsonValueFromJsonString(message.msg_setting_.yidun_anti_cheating_);
        if (values.isObject()) {
    ...
    flutter::EncodableMap attachMap;
    YXLOG(Info) << "convertNimMessageToDartMessage type " << message.msg_type_ << YXLOGEnd;
    if (message.msg_type_ == nim_chatroom::kNIMChatRoomMsgTypeCustom) {
        YXLOG(Info) << "convertNimMessageToDartMessage msg_attach_ " << message.msg_attach_ << YXLOGEnd;
        nim_cpp_wrapper_util::Json::Value values = Convert::getInstance()->getJsonValueFromJsonString(message.msg_attach_);
    ...
    nim_cpp_wrapper_util::Json::Value value = Convert::getInstance()->getJsonValueFromJsonString(message.from_ext_);
