...
void FLTAudioRecorderService::onMethodCalled(const std::string& method, const flutter::EncodableMap* arguments,
                                             std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!checkAudioRecordInit()) {
        result->Error("", "", NimResult::getErrorResult(-1, "audioRecorder init failed"));
        return;
    }
...
bool FLTAudioRecorderService::checkAudioRecordInit() {
    ...
    nim_audio::Audio::RegStartCaptureCb(&FLTAudioRecorderService::startCaptureCallback);
    nim_audio::Audio::RegStopCaptureCb(&FLTAudioRecorderService::stopCaptureCallback);
    nim_audio::Audio::RegCancelAudioCb(&FLTAudioRecorderService::cancelRecordCallback);
    return true;
}

void FLTAudioRecorderService::startRecord(const flutter::EncodableMap* arguments,
                                          std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    for (iter; iter != arguments->end(); ++iter) {
        ...
        if (iter->first == flutter::EncodableValue("recordState")) {
            ...
            Convert::getInstance()->convertDartStringToNIMEnum(strRecordType, Convert::getInstance()->getAudioOutputFormat(), audioType);
...
void FLTAudioRecorderService::stopRecord(std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
...
void FLTAudioRecorderService::cancelRecord(std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
...
void FLTAudioRecorderService::startCaptureCallback(int code) {
    if (code == 200) {
        ...
        FLTService::notifyEventEx("AudioRecorderService", "onRecordStart", arguments);
    }
}

void FLTAudioRecorderService::stopCaptureCallback(int code, const char* call_id, const char* res_id, const char* file_path, const char* file_ext,
                                                  long file_size, int audio_duration) {
    if (code == 200) {
        flutter::EncodableMap arguments;
        arguments.insert(std::make_pair("filePath", file_path));
        FLTService::notifyEventEx("AudioRecorderService", "onRecordSuccess", arguments);
    }
}
...
void FLTAudioRecorderService::cancelRecordCallback(int code) {
    if (code == 200) {
        flutter::EncodableMap arguments;
        FLTService::notifyEventEx("AudioRecorderService", "onRecordCancel", arguments);
    }
}
#endif
