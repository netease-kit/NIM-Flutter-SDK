...
void NimCore::onMethodCall(const std::string& method, const flutter::EncodableMap* arguments,
    ...
    if (serviceName_iter != arguments->end() && !serviceName_iter->second.IsNull()) {
        ...
        if (service) {
            std::shared_ptr<MockMethodResult> mockResult = std::make_shared<MockMethodResult>(serviceName, method, result);
            YXLOG_API(Info) << "mn: " << method << ", args: " << Convert::getInstance()->getStringFormMapForLog(arguments) << YXLOGEnd;
...
void NimCore::invokeMethod(const std::string& method, const flutter::EncodableMap& arguments) {
    if (m_channel) {
        m_channel->InvokeMethod(method, std::make_unique<flutter::EncodableValue>(arguments));
    }
}
...
NimCore::NimMethodChannel* NimCore::getMethodChannel() {
    return m_channel;
}
...
MockMethodResult::MockMethodResult(const std::string serviceName, const std::string methodName,
                                   std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
    : m_serviceName(serviceName), m_methodName(methodName), m_result(std::move(result)) {
}
