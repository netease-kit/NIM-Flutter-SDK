    ...
    if (s_curMoudleName.empty()) {
        ...
        if (0 != dw) {
            ...
            if (curMoudleName.empty()) {
                ...
                int size_needed = WideCharToMultiByte(CP_UTF8, 0, &curMoudleName[0], (int)curMoudleName.size(), NULL, 0, NULL, NULL);
                std::string strTo(size_needed, 0);
                WideCharToMultiByte(CP_UTF8, 0, &curMoudleName[0], (int)curMoudleName.size(), &strTo[0], size_needed, NULL, NULL);
...
static bool IsCurMoudleCrash(MyStackWalker& stackwalker) {
    if (stackwalker.HasModule(getCurMoudleName()))
        return true;
    return false;
}
...
LONG WINAPI MyUnhandledExceptionFilter(EXCEPTION_POINTERS* exp) {
    ...
    if (IsCurMoudleCrash(stackwalker)) {
        ...
        std::string all_stacks = stackwalker.GetCallStackInfoTxt(getCurMoudleName(), cur_stacks);
        all_stacks.insert(0, "\n");
...
void InitDumpInfo(const std::string& dumpFile) {
    if (dumpFile.empty()) {
        ...
        int size_needed = MultiByteToWideChar(CP_UTF8, 0, &dumpFile[0], (int)dumpFile.size(), NULL, 0);
        std::wstring wstrTo(size_needed, 0);
        MultiByteToWideChar(CP_UTF8, 0, &dumpFile[0], (int)dumpFile.size(), &wstrTo[0], size_needed);
        dump_path_ = wstrTo;
    }
