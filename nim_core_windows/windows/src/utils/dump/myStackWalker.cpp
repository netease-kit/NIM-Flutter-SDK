...
CallStackItem::CallStackItem(MyStackWalker::CallstackEntry& entry) {
    ...
    if (entry.lineFileName[0] != 0)
        sourceFile_ = entry.lineFileName;
...
std::string MyStackWalker::GetCallStackInfoTxt(const std::string& dll_name, std::string& stacks) {
    std::string txt;
    for (auto& item : callStack_) {
...
void MyStackWalker::OnLoadModule(LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size, DWORD result, LPCSTR symType, LPCSTR pdbName,
