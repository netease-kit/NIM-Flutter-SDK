...
#define ALOG_DIY(moduleName, type, level) ALog().Init(moduleName, __FILE__, __LINE__, type, level)
...
public:
    ALog& Init(const char* module, const char* file, long line, ALogType type, ALogLevel level);
...
public:
    static ALog* CreateInstance(const std::string& filePath, const std::string& fileName, ALogLevel minLevel);
...
private:
    explicit ALog(const std::string& file, const std::string& fileName, ALogLevel minLevel);
    void write();
