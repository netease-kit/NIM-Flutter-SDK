...
class MethodChannelSignallingService extends SignallingServicePlatform {
  /// 在线事件回调
  final _onOnlineEvent = StreamController<NIMSignallingEvent>.broadcast();

  /// 离线事件回调
  final _onOfflineEvent =
      StreamController<List<NIMSignallingEvent>>.broadcast();

  /// 多端事件回调
  final _onMultiClientEvent = StreamController<NIMSignallingEvent>.broadcast();

  /// 同步还在的信令频道房间列表
  final _onSyncRoomInfoList =
  ...
  Future onEvent(String method, arguments) {
    switch (method) {
      case 'onOnlineEvent':
        ...
        _onOnlineEvent.add(
            NIMSignallingEvent.fromJson(Map<String, dynamic>.from(arguments)));
        break;
      case 'onOfflineEvent':
        var messageList = arguments['offlineEvents'] as List<dynamic>?;
        List<NIMSignallingEvent>? list = messageList
            ?.map((e) =>
                NIMSignallingEvent.fromJson(Map<String, dynamic>.from(e)))
            .toList();
        if (list != null) {
          _onOfflineEvent.add(list);
        }
        break;
      case 'onMultiClientEvent':
        assert(arguments is Map);
        _onMultiClientEvent.add(
            NIMSignallingEvent.fromJson(Map<String, dynamic>.from(arguments)));
        break;
      case 'onSyncRoomInfoList':
        var messageList = arguments['syncRoomInfoList'] as List<dynamic>?;
        List<NIMSignallingRoomInfo>? list = messageList
            ?.map((e) =>
                NIMSignallingRoomInfo.fromJson(Map<String, dynamic>.from(e)))
            .toList();
        if (list != null) {
          _onSyncRoomInfoList.add(list);
        }
        break;
      default:
  ...
  Future<NIMResult<NIMSignallingCallResult>> call(
      NIMSignallingCallParams params) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'call',
        arguments: {'params': params.toJson()},
      ),
      convert: (map) => NIMSignallingCallResult.fromJson(map),
    );
  }
  ...
  Future<NIMResult<NIMSignallingCallSetupResult>> callSetup(
      NIMSignallingCallSetupParams params) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'callSetup',
        arguments: {'params': params.toJson()},
      ),
      convert: (map) => NIMSignallingCallSetupResult.fromJson(map),
    );
  }
  ...
  Future<NIMResult<NIMSignallingChannelInfo>> createRoom(
      ...
      String? channelName,
      String? channelExtension) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'createRoom',
        arguments: {
          'channelType': getNIMSignallingChannelTypeValue(channelType),
          'channelName': channelName,
          'channelExtension': channelExtension
        },
      ),
      convert: (map) => NIMSignallingChannelInfo.fromJson(map),
    );
  }
  ...
  Future<NIMResult<void>> closeRoom(
      String channelId, bool? offlineEnabled, String? serverExtension) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'closeRoom',
        arguments: {
  ...
  Future<NIMResult<V2NIMSignallingJoinResult>> joinRoom(
      NIMSignallingJoinParams params) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'joinRoom',
        arguments: {'params': params.toJson()},
      ),
      convert: (map) => V2NIMSignallingJoinResult.fromJson(map),
    );
  }
  ...
  Future<NIMResult<void>> cancelInvite(
      NIMSignallingCancelInviteParams params) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'cancelInvite',
        arguments: {'params': params.toJson()},
  ...
  Future<NIMResult<void>> rejectInvite(
      NIMSignallingRejectInviteParams params) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'rejectInvite',
        arguments: {'params': params.toJson()},
  ...
  Future<NIMResult<void>> acceptInvite(
      NIMSignallingAcceptInviteParams params) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'acceptInvite',
        arguments: {'params': params.toJson()},
  ...
  @override
  Future<NIMResult<void>> sendControl(String channelId,
      String receiverAccountId, String? serverExtension) async {
    return NIMResult.fromMap(
      await invokeMethod(
        'sendControl',
        arguments: {
  ...
  Future<NIMResult<NIMSignallingRoomInfo>> getRoomInfoByChannelName(
    ...
    return NIMResult.fromMap(
        ...
        convert: (map) => NIMSignallingRoomInfo.fromJson(map));
  }

  @override
  Stream<NIMSignallingEvent> get onMultiClientEvent =>
      _onMultiClientEvent.stream;

  @override
  Stream<List<NIMSignallingEvent>> get onOfflineEvent => _onOfflineEvent.stream;

  @override
  Stream<NIMSignallingEvent> get onOnlineEvent => _onOnlineEvent.stream;

  @override
  Stream<List<NIMSignallingRoomInfo>> get onSyncRoomInfoList =>
      _onSyncRoomInfoList.stream;
}
