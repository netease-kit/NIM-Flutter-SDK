...
public:
    ...
    void setMethodChannel(void* pChannel) {
        m_channel = (__bridge id)pChannel;
    }
    ...
    void invokeMethod(const std::string& eventName, const flutter::EncodableMap& arguments) {
        ...
        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData
                                                            options:NSJSONReadingMutableContainers
                                                              error:&err];
        //NSLog(@"invokeMethod eventName: %@", [NSString stringWithCString:eventName.c_str() encoding:NSUTF8StringEncoding]);
        //NSLog(@"invokeMethod dic: %@", dic);
...
void NimCore::setAppkey(const std::string& appkey) {
    m_appKey = appkey;
}

std::string NimCore::getAppkey() const {
    return m_appKey;
}

void NimCore::setLogDir(const std::string& logDir) {
    m_logDir = logDir;
}
std::string NimCore::getLogDir() const {
    return m_logDir;
}
...
void NimCore::setMethodChannel(void* pChannel) {
    m_methodChannel->setMethodChannel(pChannel);
}
...
void NimCore::onMethodCall(const std::string& methodName, const flutter::EncodableMap& arguments, const NimResultCallback& resultCallback) {
    ...
    auto serviceName_iter = arguments.find(flutter::EncodableValue("serviceName"));
        if (serviceName_iter != arguments.end() && !serviceName_iter->second.IsNull()) {
            ...
            if (service) {
                std::shared_ptr<MockMethodResult> mockResult = std::make_shared<MockMethodResult>(serviceName, methodName, resultCallback);
                YXLOG_API(Info) << "mn: " << methodName
                                << ", args: " << Convert::getInstance()->getStringFormMapForLog(&arguments) << YXLOGEnd;
...
void MockMethodResult::ErrorInternal(const std::string& error_code, const std::string& error_message, const flutter::EncodableValue* details) {
    YXLOG_API(Warn) << "cb error, sn: " << m_serviceName << ", mn: " << m_methodName << ", error_code: " << error_code
                    << ", error_msg: " << error_message << ", details: " << getStringFormEncodableValue(details) << YXLOGEnd;
    if (m_resultCallback)
        m_resultCallback(details, false);
}

void MockMethodResult::NotImplementedInternal() {
    YXLOG_API(Warn) << "cb notImplemented, sn: " << m_serviceName << ", mn: " << m_methodName << YXLOGEnd;
    if (m_resultCallback)
        m_resultCallback(nullptr, true);
}
...
void MockMethodResult::SuccessInternal(const flutter::EncodableValue* result) {
    ...
    if (m_resultCallback)
        m_resultCallback(result, false);
}

std::string MockMethodResult::getStringFormEncodableValue(const flutter::EncodableValue* value) const {
    if (!value) {
        return "";
    }
