...
FLTPassThroughService::FLTPassThroughService() {
    ...
    nim::PassThroughProxy::RegReceivedHttpMsgCb(
        std::bind(&FLTPassThroughService::receivedHttpMsgCb, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3), "");
}

void FLTPassThroughService::onMethodCalled(const std::string& method, const flutter::EncodableMap* arguments,
...
void FLTPassThroughService::httpProxy(const flutter::EncodableMap* arguments,
                                      std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
    if (!arguments) {
        return;
    }
    ...
    nim::PassThroughProxy::SendHttpRequest(
        zone, path, static_cast<nim::NIMSendHttpRequestMethods>(method), header, body, "",
        [=](int res_code, const std::string& strHeader, const std::string& strBody, const std::string& json_extension) {
            if (res_code == 200) {
                ...
                result->Error("", "", NimResult::getErrorResult(res_code, "httpProxy failed"));
            }
        });
}

void FLTPassThroughService::receivedHttpMsgCb(const std::string& from_accid, const std::string& body, uint64_t timestamp) {
    flutter::EncodableMap arguments;
    flutter::EncodableMap data;
