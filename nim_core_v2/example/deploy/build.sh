...
ARCHIVE_DIRECTOR_NAME="im_flutter_v2_example_${now}_${BUILD_COMMIT_ID}_${BUILD_EVN}"
ARCHIVE_NAME="im_flutter_v2_example_${now}_${BUILD_COMMIT_ID}_${BUILD_EVN}"
...
if [ "$BACKUP_DIR" != "" ] && [ "$BACKUP_DIR" != "--" ]; then
    ...
    target_path="outputs/${BACKUP_DIR}/im_flutter_v2_example/${VERSION_NAME}"
    mkdir -p "${target_path}"
    if [ "$PLATFORM" == "Android" ]; then
...
directory_name="${VERSION_NAME}"
platform_lowercase=$(echo "$PLATFORM" | tr '[A-Z]' '[a-z]')
download_url=http://10.242.141.186/xkit/$BACKUP_DIR/im_flutter_v2_example/$VERSION_NAME/$platform_lowercase/$ARCHIVE_NAME
