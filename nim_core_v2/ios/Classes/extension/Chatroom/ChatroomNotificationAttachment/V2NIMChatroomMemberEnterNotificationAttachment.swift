...
extension V2NIMChatroomMemberEnterNotificationAttachment {
  /// 转换为对象， 用keypath 取属性作为 key 值
  /// - Returns: 对象
  static func fromDictionary(_ arguments: [String: Any]) -> V2NIMChatroomMemberEnterNotificationAttachment {
    let attach = V2NIMChatroomMemberEnterNotificationAttachment()
    ...
    if let chatBanned = arguments["chatBanned"] as? Bool {
      attach.setValue(chatBanned, forKeyPath: #keyPath(V2NIMChatroomMemberEnterNotificationAttachment.chatBanned))
    }

    if let tempChatBanned = arguments["tempChatBanned"] as? Bool {
      attach.setValue(tempChatBanned, forKeyPath: #keyPath(V2NIMChatroomMemberEnterNotificationAttachment.tempChatBanned))
    }

    if let tempChatBannedDuration = arguments["tempChatBannedDuration"] as? Int {
      attach.setValue(TimeInterval(tempChatBannedDuration / 1000), forKeyPath: #keyPath(V2NIMChatroomMemberEnterNotificationAttachment.tempChatBannedDuration))
    }
