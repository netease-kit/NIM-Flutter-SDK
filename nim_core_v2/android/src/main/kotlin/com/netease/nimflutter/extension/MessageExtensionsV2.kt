...
import com.netease.nimlib.sdk.v2.message.config.V2NIMMessageStreamConfig
...
import com.netease.nimlib.sdk.v2.message.enums.V2NIMMessageStreamStatus
...
import com.netease.nimlib.sdk.v2.message.model.V2NIMMessageStreamChunk
...
fun Map<String, *>.toMessage(): V2NIMMessage =
    V2NIMMessageBuilder
        ...
        .topicRefer((this["topicRefer"] as? Map<String, Any?>)?.toMessageTopicRefer())
        .build()
...
fun V2NIMMessage.toMap(): Map<String, Any?> =
    mapOf(
        ...
        "topicRefer" to this.topicRefer?.toMap(),
        ...
        "streamConfig" to this.streamConfig?.toMap(),
        "messageSource" to this.messageSource
    )
...
fun Map<String, *>.toMessageTopicRefer(): V2NIMTopicRefer {
    val map = this
...
fun V2NIMTopicRefer.toMap(): Map<String, Any?> =
    mapOf(
        "conversationId" to this.conversationId,
        "topicId" to this.topicId,
        "createTime" to this.createTime
    )
...
fun Map<String, *>.toMessageAIConfig(): V2NIMMessageAIConfig =
    V2NIMMessageAIConfig().apply {
        ...
        this.airaGs =
            (this@toMessageAIConfig["aiRAGs"] as? List<Map<String, Any?>>)?.map {
                it.toV2NIMAIRAGInfo()
            }
...
fun V2NIMMessageStreamConfig.toMap(): Map<String, Any?> =
    mapOf(
        "status" to this.status?.value,
        "lastChunk" to this.lastChunk?.toMap(),
        "rags" to this.rags?.map { it.toMap() }?.toList()
    )

fun V2NIMMessageStreamChunk.toMap(): Map<String, Any?> =
    mapOf(
        "content" to content,
        "messageTime" to messageTime,
        "chunkTime" to chunkTime,
...
fun Map<String, *>.toV2NIMMessageStreamConfig(): V2NIMMessageStreamConfig =
    V2NIMMessageStreamConfig(
        V2NIMMessageStreamStatus.typeOfValue(this["status"] as? Int ?: 0),
        (this["lastChunk"] as? Map<String, Any?>)?.toV2NIMMessageStreamChunk(),
        (this["rags"] as? List<Map<String, Any?>>)?.map { it.toV2NIMAIRAGInfo() }
    )

fun Map<String, *>.toV2NIMMessageStreamChunk(): V2NIMMessageStreamChunk =
...
fun Map<String, *>.toModifyMessageParams(): V2NIMModifyMessageParams =
    V2NIMModifyMessageParams().apply {
        ...
        this.isClientAntispamEnabled = this@toModifyMessageParams["clientAntispamEnabled"] as? Boolean ?: false
        this.clientAntispamReplace = this@toModifyMessageParams["clientAntispamReplace"] as? String
    }
