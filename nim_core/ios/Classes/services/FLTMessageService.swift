...
//    }
    ...
    func onMethodCalled(_ method: String, _ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        case MessageType.DownloadAttachment.rawValue:
            if let message = NIMMessage.convertToMessage(arguments) {
                fetchMessageAttachment(message, resultCallback)
            }
    ...
    private func voiceToText(_ arguments: [String : Any], _ resultCallback: ResultCallback){
        ...
        if let message = NIMMessage.yx_model(with: arguments), let attachment = message.messageObject as? NIMAudioObject {
            let option = NIMAudioToTextOption()
            option.filepath = attachment.path ?? ""
