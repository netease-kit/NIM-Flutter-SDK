...
class FLTTeamService: FLTBaseService, FLTService {
    ...
    func serviceName() -> String {
        return ServiceType.TeamService.rawValue
    }
    ...
    func onMethodCalled(_ method: String, _ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        case TeamType.RemoveManagers.rawValue: removeMembers(arguments, resultCallback)
        ...
        case TeamType.QueryMutedTeamMembers.rawValue: queryMutedTeamMembers(arguments, resultCallback)
        ...
        case TeamType.UpdateMyMemberExtension.rawValue: updateMyMemberExtension(arguments, resultCallback)
        default:
    ...
    private func createTeam(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let option = arguments["createTeamOptions"] as? [String : Any],
           ...
           let users = arguments["members"] as? [String] {
            ...
            NIMSDK.shared().teamManager.createTeam(createOption, users: users) { error, teamid, failedUserIds in
                if error != nil {
                    weakSelf?.teamCallback(error, nil, resultCallback)
                } else {
                    NIMSDK.shared().teamManager.fetchTeamInfo(teamid!, completion: { error, team in
                        weakSelf?.teamCallback(error, ["team":team?.toDic(), "failedInviteAccounts":failedUserIds], resultCallback)
                    });
                }
            }
        } else {
    ...
    private func rejectApply(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        weak var weakSelf = self
        NIMSDK.shared().teamManager.rejectApply(toTeam: teamId, userId: userId, rejectReason: reason) { error in
            weakSelf?.teamCallback(error, nil, resultCallback)
        }
    }
    ...
    private func addMembersEx(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        NIMSDK.shared().teamManager.addUsers(users, toTeam: teamId, postscript: postscript, attach: attach) { error, members in
    ...
    private func rejectInvite(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        weak var weakSelf = self
        NIMSDK.shared().teamManager.rejectInvite(withTeam: teamId, invitorId: invitorId, rejectReason: reason) { error in
            weakSelf?.teamCallback(error, nil, resultCallback)
        }
    }
    ...
    private func getMemverInvitor(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        weak var weakSelf = self
        NIMSDK.shared().teamManager.fetchInviterAccids(teamId, withTargetMembers: memberIds) { error, inviters in
    ...
    private func updateMemberNick(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        weak var weakSelf = self
        NIMSDK.shared().teamManager.updateUserNick(userId, newNick: newNick, inTeam: teamId) { error in
            weakSelf?.teamCallback(error, nil, resultCallback)
        }
    }
    ...
    private func transferTeam(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        weak var weakSelf = self
        NIMSDK.shared().teamManager.transferManager(withTeam: teamId, newOwnerId: newOwnerId, isLeave: isLeave) { error  in
            if error != nil {
                weakSelf?.teamCallback(error, nil, resultCallback)
            } else {
                NIMSDK.shared().teamManager.fetchTeamMembers(teamId) { error, members in
    ...
    //查询禁言用户列表
    private func queryMutedTeamMembers(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        guard let teamId = getTeamId(arguments) else {
            errorCallBack(resultCallback, "parameter is error")
            return
        }
    ...
    private func updateTeamFields(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        if requestList.contains("announcement") {
            values[NSNumber.init(value: NIMTeamUpdateTag.anouncement.rawValue)] = request["announcement"] as? String ?? ""
        }
        if requestList.contains("beInviteMode"),
           let beInviteMode = request["beInviteMode"] as? Int {
            values[NSNumber.init(value: NIMTeamUpdateTag.beInviteMode.rawValue)] = String(beInviteMode)
        }
        if requestList.contains("extension") {
            values[NSNumber.init(value: NIMTeamUpdateTag.clientCustom.rawValue)] = request["extension"] as? String ?? ""
        }
        if requestList.contains("icon") {
            values[NSNumber.init(value: NIMTeamUpdateTag.avatar.rawValue)] = request["icon"] as? String ?? ""
        }
        if requestList.contains("introduce") {
            values[NSNumber.init(value: NIMTeamUpdateTag.intro.rawValue)] = request["introduce"] as? String ?? ""
        }
        if requestList.contains("inviteMode"),
           let inviteMode = request["inviteMode"] as? Int {
            values[NSNumber.init(value: NIMTeamUpdateTag.inviteMode.rawValue)] = String(inviteMode)
        }
        ...
        if requestList.contains("name") {
            values[NSNumber.init(value: NIMTeamUpdateTag.name.rawValue)] = request["name"] as? String ?? ""
        }
        if requestList.contains("teamExtensionUpdateMode"),
           let teamExtensionUpdateMode = request["teamExtensionUpdateMode"] as? Int {
            values[NSNumber.init(value: NIMTeamUpdateTag.updateClientCustomMode.rawValue)] = String(teamExtensionUpdateMode)
        }
        if requestList.contains("teamUpdateMode"),
           let teamUpdateMode = request["teamUpdateMode"] as? Int {
            values[NSNumber.init(value: NIMTeamUpdateTag.updateInfoMode.rawValue)] = String(teamUpdateMode)
        }
        if requestList.contains("verifyType"),
           let verifyType = request["verifyType"] as? Int {
            values[NSNumber.init(value: NIMTeamUpdateTag.joinMode.rawValue)] = String(verifyType)
        }
    ...
    //群组检索
    private func searchTeamWithOption(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
    ...
    private func updateMyMemberExtension(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        guard let teamId = getTeamId(arguments),
...
extension FLTTeamService: NIMTeamManagerDelegate {
    ...
    func onTeamMemberUpdated(_ team: NIMTeam, withMembers memberIDs: [String]?) {
        if team.type == .super {
            notifyEvent(ServiceType.SuperTeamService.rawValue, "onSuperTeamMemberUpdate", ["teamMemberList" : superTeamMembers(memberIDs, team.teamId)])
        } else {
            notifyEvent(serviceName(), "onTeamMemberUpdate", ["teamMemberList" : teamMembers(memberIDs, team.teamId)])
        }
    }
    
    func onTeamMemberRemoved(_ team: NIMTeam, withMembers memberIDs: [String]?) {
        if team.type == .super {
            notifyEvent(ServiceType.SuperTeamService.rawValue, "onSuperTeamMemberRemove", ["teamMemberList" : superTeamMembers(memberIDs, team.teamId)])
        } else {
            notifyEvent(serviceName(), "onTeamMemberRemove", ["teamMemberList" : teamMembers(memberIDs, team.teamId)])
        }
    }
    ...
    func onTeamRemoved(_ team: NIMTeam) {
        if team.type == .super {
            notifyEvent(ServiceType.SuperTeamService.rawValue, "onSuperTeamRemove", ["team" : team.toDic() as Any])
        } else {
    ...
    func onTeamUpdated(_ team: NIMTeam) {
        if team.type == .super {
            notifyEvent(ServiceType.SuperTeamService.rawValue, "onSuperTeamUpdate", ["teamList" : [team.toDic() as Any]])
        } else {
