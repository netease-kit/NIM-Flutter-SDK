...
//    }
    ...
    func serviceName() -> String {
        return ServiceType.SystemNotificationService.rawValue
    }
    ...
    // MARK: Public Method
    func querySystemMessagesIOSAndDesktop(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        let limit = arguments["limit"] as? Int ?? 0
        let notification = NIMSystemNotification.init()
        if let systemMessage = arguments["systemMessage"] as? [String : Any],
           let time = systemMessage["time"] as? Int {
            ...
            notification.setValue(Date.init().timeIntervalSince1970, forKeyPath: "timestamp")
        }
        if let systemMessageList = NIMSDK.shared().systemNotificationManager.fetchSystemNotifications(notification, limit: limit) {
    ...
    func querySystemMessageByTypeIOSAndDesktop(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        let limit = arguments["limit"] as? Int ?? 0
        let notification = NIMSystemNotification.init()
        if let systemMessage = arguments["systemMessage"] as? [String : Any],
           let time = systemMessage["time"] as? Int {
        ...
        var filter: NIMSystemNotificationFilter? = nil
        if let systemMessageTypeList = arguments["systemMessageTypeList"] as? [String] {
            ...
            filter!.notificationTypes = systemMessageTypeList.map({ type in
                NSNumber.init(value: convertType(type: type).rawValue)
            })
        }
        if let systemMessageList = NIMSDK.shared().systemNotificationManager.fetchSystemNotifications(notification, limit: limit, filter: filter) {
            let result = NimResult(["systemMessageList" : systemMessageList.map({ noti in
                noti.toDic()
            })], 0, nil)
            resultCallback.result(result.toDic())
    ...
    func querySystemMessageUnread(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        let systemMessageList = NIMSDK.shared().systemNotificationManager.fetchSystemNotifications(nil, limit: 0)
    ...
    func querySystemMessageUnreadCount(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        let systemMessageList = NIMSDK.shared().systemNotificationManager.allUnreadCount()
        let result = NimResult(systemMessageList, 0, nil)
        resultCallback.result(result.toDic())
    }
    
    func querySystemMessageUnreadCountByType(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let systemMessageTypeList = arguments["systemMessageTypeList"] as? [String] {
            let filter = NIMSystemNotificationFilter()
            filter.notificationTypes = systemMessageTypeList.map({ type in
                NSNumber.init(value: Int(type) ?? -1)
            })
    ...
    func resetSystemMessageUnreadCount(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        NIMSDK.shared().systemNotificationManager.markAllNotificationsAsRead()
        let result = NimResult(nil, 0, nil)
        resultCallback.result(result.toDic())
    }
    
    func resetSystemMessageUnreadCountByType(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let systemMessageTypeList = arguments["systemMessageTypeList"] as? [String] {
            let filter = NIMSystemNotificationFilter()
            filter.notificationTypes = systemMessageTypeList.map({ type in
                NSNumber.init(value: Int(type) ?? -1)
    ...
    func sendCustomNotification(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let notification = arguments["customNotification"] as? [String : Any] {
            ...
            let session = NIMSession.init(sessionId, type: sessionType ?? NIMSessionType.P2P)
            NIMSDK.shared().systemNotificationManager.sendCustomNotification(NIMCustomSystemNotification.createCustomSystemNotification(notification)!, to: session) { error in
                if error != nil {
