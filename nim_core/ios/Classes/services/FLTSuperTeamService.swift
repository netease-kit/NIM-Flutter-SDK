...
class FLTSuperTeamService: FLTBaseService, FLTService {
    func serviceName() -> String {
        return ServiceType.SuperTeamService.rawValue
    }
    ...
    func onMethodCalled(_ method: String, _ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        case SuperTeamType.QueryMemberListByPage.rawValue: queryMemberListByPage(arguments, resultCallback)
        ...
        case SuperTeamType.UpdateMyMemberExtension.rawValue: updateMyMemberExtension(arguments, resultCallback)
        ...
        case SuperTeamType.QueryMutedTeamMembers.rawValue: queryMutedTeamMembers(arguments, resultCallback)
        ...
        case SuperTeamType.SearchTeamIdByName.rawValue: searchTeamWithOption(arguments, resultCallback)
        case SuperTeamType.SearchTeamsByKeyword.rawValue: searchTeamWithOption(arguments, resultCallback)
        default:
    ...
    func applyJoinTeam(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           let message = arguments["postscript"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.apply(toTeam: teamId, message: message) { error, status in
                if error != nil {
    ...
    func passApply(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           let userId = arguments["account"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.passApply(toTeam: teamId, userId: userId) { error, status in
    ...
    func rejectApply(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           ...
           let rejectReason = arguments["reason"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.rejectApply(toTeam: teamId, userId: userId, rejectReason: rejectReason) { error in
                weakSelf?.superTeamCallback(error, nil, resultCallback)
            }
        } else {
            errorCallBack(resultCallback, "invalid arguments")
    ...
    func addMembers(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let users = arguments["accountList"] as? [String],
           let teamId = arguments["teamId"] as? String {
            ...
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.addUsers(users, toTeam: teamId, postscript: postscript, attach: attach) { error, members in
    ...
    func acceptInvite(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           let invitorId = arguments["inviter"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.acceptInvite(withTeam: teamId, invitorId: invitorId) { error in
                weakSelf?.superTeamCallback(error, nil, resultCallback)
            }
        } else {
            errorCallBack(resultCallback, "invalid arguments")
    ...
    func declineInvite(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           ...
           let rejectReason = arguments["reason"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.rejectInvite(withTeam: teamId, invitorId: invitorId, rejectReason: rejectReason) { error in
                weakSelf?.superTeamCallback(error, nil, resultCallback)
            }
        } else {
            errorCallBack(resultCallback, "invalid arguments")
    ...
    func queryMemberList(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String {
            ...
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.fetchTeamMembers(teamId, option: option) { error, members in
                let membersMaps = members?.compactMap({ member in
                    member.toDic()
                })
                weakSelf?.superTeamCallback(error, ["teamMemberList":membersMaps], resultCallback)
    ...
    func queryTeamMember(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           let userId = arguments["account"] as? String {
            let member :NIMTeamMember? = NIMSDK.shared().superTeamManager.teamMember(userId, inTeam: teamId)
    ...
    func updateMemberNick(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           ...
           let newNick = arguments["nick"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.updateUserNick(userId, newNick: newNick, inTeam: teamId) { error in
                weakSelf?.superTeamCallback(error, nil, resultCallback)
            }
        } else {
            errorCallBack(resultCallback, "invalid arguments")
    ...
    func transferTeam(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let teamId = arguments["teamId"] as? String,
           let newOwnerId = arguments["account"] as? String {
            ...
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.transferManager(withTeam: teamId, newOwnerId: newOwnerId, isLeave: isLeave) { error in
                if error != nil {
                    weakSelf?.superTeamCallback(error, nil, resultCallback)
                } else {
                    let option = NIMTeamFetchMemberOption()
    ...
    func removeManagers(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let users = arguments["accountList"] as? [String],
           let teamId = arguments["teamId"] as? String {
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.removeManagers(fromTeam: teamId, users: users) { error in
                if error != nil {
                    weakSelf?.superTeamCallback(error, nil, resultCallback)
                } else {
                    let option = NIMTeamFetchMemberOption()
    ...
    func muteTeamMember(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        if let users = arguments["accountList"] as? [String],
           let teamId = arguments["teamId"] as? String {
            ...
            weak var weakSelf = self
            NIMSDK.shared().superTeamManager.updateMuteState(mute, userIds: users, inTeam: teamId) { error in
                weakSelf?.superTeamCallback(error, nil, resultCallback)
            }
        } else {
            errorCallBack(resultCallback, "invalid arguments")
    ...
    func updateTeamFields(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        if requestList.contains("announcement") {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.anouncement.rawValue)] = request["announcement"] as? String ?? ""
        }
        if requestList.contains("beInviteMode"),
           let beInviteMode = request["beInviteMode"] as? Int {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.beInviteMode.rawValue)] = String(beInviteMode)
        }
        if requestList.contains("extension") {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.clientCustom.rawValue)] = request["extension"] as? String ?? ""
        }
        if requestList.contains("icon") {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.avatar.rawValue)] = request["icon"] as? String ?? ""
        }
        if requestList.contains("introduce") {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.intro.rawValue)] = request["introduce"] as? String ?? ""
        }
        if requestList.contains("name") {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.name.rawValue)] = request["name"] as? String ?? ""
        }
        if requestList.contains("verifyType"),
           let verifyType = request["verifyType"] as? Int {
            values[NSNumber.init(value: NIMSuperTeamUpdateTag.joinMode.rawValue)] = String(verifyType)
        }
    ...
    func muteTeam(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        ...
        weak var weakSelf = self
        NIMSDK.shared().superTeamManager.update(state.convertNIMNotifyState(), inTeam: teamId) { error in
            weakSelf?.superTeamCallback(error, nil, resultCallback)
        }
    }
    ...
    func notifyStateForNewMsg(_ arguments: [String : Any], _ resultCallback: ResultCallback) {
        let teamId = arguments["teamId"] as! String
        let state: NIMTeamNotifyState = NIMSDK.shared().superTeamManager.notifyState(forNewMsg: teamId)
    }
    ...
    private func superTeamCallback(_ error: Error?, _ data: Any?, _ resultCallback: ResultCallback){
