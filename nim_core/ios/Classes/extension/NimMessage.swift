...
class NIMMessageThreadOption: NSObject {
    ...
    func toDic() -> [String : Any]? {
        if var dic = self.yx_modelToJSONObject() as? [String : Any] {
            ...
            dic["replyMessageTime"] = repliedMessageTime?.intValue ?? 0
            dic["threadMessageTime"] = threadMessageTime?.intValue ?? 0
            return dic
        }
        return nil
    }
...
extension NIMMessage {
    ...
    func toDic() -> [String : Any]? {
        if var arguments = self.yx_modelToJSONObject() as? [String : Any] {
            ...
            arguments["status"] = FLT_NIMMessageStatus.convertFLTStatus(status)?.rawValue
            ...
            if let attachment = messageObject as? NSObject {
                if let convert = attachment as? NimDataConvertProtrol {
                    ...
                    arguments["messageAttachment"] = customObject.data
            ...
            if let timestamp = arguments["timestamp"] as? NSNumber {
                arguments["timestamp"] = timestamp.intValue
            }
            ...
            if session?.sessionType == .chatroom,
               let ext = messageExt as? NIMMessageChatroomExtension {
                arguments["extension"] = ["nickname":ext.roomNickname, "avatar":ext.roomAvatar, "senderExtension":ext.roomExt];
            }
    ...
    func dealNotificationObject(_ object: NIMNotificationObject) -> [String : Any] {
        ...
        case .chatroom:
            if let chatroomNotify = object.content as? NIMChatroomNotificationContent {
                ...
                target["extension"] = chatroomNotify.notifyExt
                ...
                if chatroomNotify.eventType == .enter {
                    if let obj = chatroomNotify.ext as? [String:Any] {
                        ...
                        target["tempMutedDuration"] = obj[NIMChatroomEventInfoTempMutedDurationKey]
                    }
                    ...
                    if let obj = chatroomNotify.ext as? [String:Any] {
                        ...
                        target["queueChangeType"] = FLT_NIMChatroomQueueChangeType.convert(NIMChatroomQueueChangeType.init(rawValue: obj[NIMChatroomEventInfoQueueChangeItemsKey] as! Int) ?? .invalid).rawValue
                    }
