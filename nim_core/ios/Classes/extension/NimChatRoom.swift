...
extension NIMChatroomEnterRequest: NimDataConvertProtrol {
    ...
    @objc static func modelPropertyBlacklist() -> [String] {
        return [#keyPath(NIMChatroomEnterRequest.roomExt),#keyPath(NIMChatroomEnterRequest.roomNotifyExt), #keyPath(NIMChatroomEnterRequest.tags)]
    }
...
extension NIMChatroom: NimDataConvertProtrol {
    ...
    dynamic var flt_queueModificationLevel: FLT_QueueModificationLevel? {
        get {
            if let T = objc_getAssociatedObject(self, UnsafeRawPointer.init(bitPattern: "flt_queueModificationLevel".hashValue)!) as? FLT_QueueModificationLevel {
                return T
            }else {
                return nil
            }
        ...
        set {
            objc_setAssociatedObject(self, UnsafeRawPointer.init(bitPattern: "flt_queueModificationLevel".hashValue)!, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
...
//    }
    ...
    static func fromDic(_ json: [String : Any]) -> Any? {
        if let model = NIMChatroom.yx_model(with: json) {
            if let level = json["queueModificationLevel"] as? String, let flt_level = FLT_QueueModificationLevel(rawValue: level) {
...
extension NIMChatroomMember: NimDataConvertProtrol {
    ...
    func toDic(roomId: String?) -> [String : Any]? {
        if var jsonObject = self.yx_modelToJSONObject() as? [String : Any]  {
            ...
            jsonObject["enterTime"] = Int.init(enterTimeInterval)
            return jsonObject
        }
        return nil
    }
        ...
        if let model = NIMHistoryMessageSearchOption.yx_model(with: json) {
            if let messageTypeList = json["messageTypeList"] as? [String] {
                ...
                messageTypeList.forEach { value in
                    if let type = FLT_NIMMessageType(rawValue: value)?.convertToNIMMessageType()?.rawValue{
