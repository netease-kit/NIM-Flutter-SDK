...
extension NIMSystemNotification {
    ...
    func toDic() -> [String : Any]? {
        if var jsonObject = self.yx_modelToJSONObject() as? [String : Any]  {
            jsonObject["unread"] = !self.read
            jsonObject["time"] = Int.init(self.timestamp)
            jsonObject["messageId"] = self.value(forKeyPath:"serial")
            switch self.type {
...
extension NIMCustomSystemNotification {
    ...
    static func createCustomSystemNotification(_ args: [String : Any]) -> NIMCustomSystemNotification? {
        let notification = NIMCustomSystemNotification.yx_model(with: args)
        // 处理readonly字段
        if let notificationId = args["notificationId"] {
            notification?.setValue(notificationId, forKeyPath: #keyPath(NIMCustomSystemNotification.notificationId))
        }
        if let time = args["time"] as? Int {
            notification?.setValue(time, forKeyPath: #keyPath(NIMCustomSystemNotification.timestamp))
        }
        if let fromAccount = args["fromAccount"] {
            notification?.setValue(fromAccount, forKeyPath: #keyPath(NIMCustomSystemNotification.sender))
        }
        if let receiver = args["receiver"] {
            notification?.setValue(receiver, forKeyPath: #keyPath(NIMCustomSystemNotification.receiver))
        }
        if let receiverType = args["receiverType"] {
            notification?.setValue(receiverType, forKeyPath: #keyPath(NIMCustomSystemNotification.receiverType))
        }
        if let content = args["content"] {
            notification?.setValue(content, forKeyPath: #keyPath(NIMCustomSystemNotification.content))
        }
        return notification
    }
    ...
    func toDic() -> [String : Any]? {
        if var jsonObject = self.yx_modelToJSONObject() as? [String : Any] {
            jsonObject["sessionType"] = FLT_NIMSessionType.convertFLTSessionType(self.receiverType)?.rawValue
            jsonObject["sessionId"] = self.sender
